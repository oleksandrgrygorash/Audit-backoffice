...
1. Створити нову Google-таблицю. Скопіювати її **ID** з URL (частина між
   `/d/` і `/edit`). Вставити цей ID у `Code.gs` у рядок `var SHEET_ID = '...'`.
   (Якщо скрипт створено через **Extensions → Apps Script** з тієї ж таблиці,
   `SHEET_ID` можна лишити порожнім.)
2. Вставити вміст `Code.gs` у редактор, зберегти.
